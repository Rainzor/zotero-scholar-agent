...
- **Chat-first Agent Actions (RATIFIED, ADR 0015):** Chat is the authorization boundary for special research operations. Versioned Action Cards persist lifecycle, request snapshot, capabilities, result/error, target, and Vault receipt. Structured action turns are independent and read-only; the plugin owns writes and scoped commits. Delivered actions include `/note`, deterministic `/rate 1..5`, and structured `/depth L0|L1|L2`; completed persistent actions use HEAD-gated compensating Undo. Memory shows Rating and Depth read-only.
- Vault layout:
