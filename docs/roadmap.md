...
> Track: both — milestones are cross-cutting by design; see individual ADRs
> for per-decision frontend/backend/both tags.
...
- Alternatively, when the user asks the first question and `memory.md` is still an empty
  skeleton, initialize it first. **Gap found 2026-07-13**: this branch was never
  implemented — `research-turn/orchestrator.ts` and `sidebar.ts` have no check for an
  empty skeleton before routing the question to Codex. Asking a question on an unbuilt
  L1 record currently lets Codex free-form a structured draft (observed: raw
  `TL;DR`/`algorithm_flow` JSON) directly into the visible chat answer instead of either
  auto-initializing or redirecting to the Build button. See
  `docs/plans/ui-review-backlog.md` G1 for the concrete repro and fix options.
