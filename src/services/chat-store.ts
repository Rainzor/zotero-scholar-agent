...
import type { AgentActionCard } from "./chat-actions/types";
...
export class ChatStore {
  ...
  addMessage(itemId: number, message: ChatMessage, sessionId?: string) {
    if (itemId <= 0) return;
    const state = this.getOrCreateItemState(itemId);
    let session = sessionId
      ? state.sessions.find((entry) => entry.sessionId === sessionId) || null
      : this.getSession(itemId);
    if (!session) {
      if (sessionId) return;
  ...
  findRunningAction(itemId: number, sessionId: string): AgentActionCard | null {
    const state = this.getItemState(itemId);
    const session = state?.sessions.find(
      (entry) => entry.sessionId === sessionId,
    );
    return (
      session?.messages.find((message) => message.action?.state === "running")
        ?.action || null
    );
  }
  ...
  needsAutoTitle(itemId: number, sessionId?: string): boolean {
    const state = this.getItemState(itemId);
